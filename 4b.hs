...
solve lines = 
  let
    ...
    (guardId, minute, count) = head $ reverse $ sortWith (\(_, _, count) -> count) $ map sleepiestMinute groupedByGuard
  in guardId * minute

--                             id,  minute, count
sleepiestMinute :: [Shift] -> (Int, Int, Int)
sleepiestMinute guardShifts = 
  let sleepiestMinuteGroup = listToMaybe $ reverse $ sortWith length $ group $ sort $ guardShifts >>= snd
      guardId = fst $ head $ guardShifts
      (count, minute) = fromMaybe (0, 0) $ fmap (\g -> (length g, head g)) sleepiestMinuteGroup
  in (guardId, minute, count)
